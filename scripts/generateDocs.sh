...
if [ -d "docs/@libra2org/ts-sdk-$npm_package_version" ]; then
...
# --out "docs/@libra2org/ts-sdk-$npm_package_version" - Specifies the output directory for the generated documentation, 
...
npx typedoc src/index.ts --options typedoc.json --out "docs/@libra2org/ts-sdk-$npm_package_version" --plugin typedoc-plugin-missing-exports --internalModule PrivateCode --cleanOutputDir --excludeInternal --includeVersion --skipErrorChecking
...
if [ -n "$LINE" ]; then
  ...
  # TODO: Make this more stable / a different way of generating that isn't just inserting a line
  {
    ...
    echo "- [$LABEL - @libra2org/ts-sdk-$npm_package_version](@libra2org/ts-sdk-$npm_package_version)";
    tail -n +$NEXT_LINE $INDEX_FILE;
  } > $INDEX_FILE.tmp && mv $INDEX_FILE.tmp $INDEX_FILE
fi

# Now update the redirect
REDIRECT_FILE='docs/@libra2org/ts-sdk-latest/index.md';
$(sed -i.bak "s/redirect_to:.*/redirect_to: \/@libra2org\/ts-sdk-${npm_package_version}/" $REDIRECT_FILE)
